...
class AuthServiceImplTest {
    ...
    void register_success_assignsUserRole_andHashesPassword() {
        ...
        when(userMapper.toResponse(any(User.class))).thenReturn(new UserResponseDto(10L, dto.email(), dto.firstName(), dto.lastName(), true, Set.of("USER")));

        UserResponseDto resp = authService.register(dto);

        assertEquals(dto.email(), resp.email());
    ...
    void changePassword_success_updatesHash() {
        ...
        User user = User.builder().id(1L).email(email).passwordHash("oldHash").isActive(true).build();
    ...
    void changePassword_fails_whenOldPasswordWrong() {
        ...
        assertThrows(IllegalArgumentException.class, () -> authService.changePassword(email, new ChangePasswordRequestDto("wrongOld", "newPass123")));
        verify(userRepository, never()).save(any());
    }
}
